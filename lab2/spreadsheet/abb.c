...
void insere_formula(arv *arvore, char *formula){ //insere no nó da árvore a formula
    if(arvore != NULL){
        arvore->formula = strdup(formula);
    }
    else{ printf("Nao foi possivel inserir formula"); exit(3);}
}

void altera_estado(arv* arvore, int est){
    if(est > 0 && est < 4) arvore->estado = est;
}
...
int maxDepth(arv* arvore){
   if (!arvore) return 0;
        ...
        if(esqDepth > dirDepth) return(esqDepth + 1);
...
/*void printaERD(arv *arvore){
    ...
    if(copy->formula) printf("%s  ", copy->formula);
    printaERD(copy->dir);
}
...
arv* arv_insere(arv **a, char *chave){
    ...
    if(!aux){
        ...
        *a = aux;
        ...
        int cmp = strcmp(chave, aux->chave);
        if(cmp < 0){
            ret = arv_insere(&(aux->esq), chave);
        }
        else if(cmp > 0){
            ret =  arv_insere(&(aux->dir), chave);
        }
        else if(cmp == 0){
            ret = NULL;
        }
